...
namespace DaprShowcase.Services.Orchestrator.Application.Dapr.Bindings
{
        ...
        IAsyncConverter<DaprStateAttribute, object?>
    {
        ...
        public async Task<byte[]> ConvertAsync(DaprStateAttribute input, CancellationToken cancellationToken)
        {
            ...
            try
            {
                bytes = json.ToObject<byte[]>() ?? new byte[0];
            }
            catch (JsonException)
            {
        ...
        async Task<DaprStateRecord> GetStateRecordAsync(DaprStateAttribute input, CancellationToken cancellationToken)
        {
            DaprStateRecord stateRecord = await this.daprClient.GetStateAsync(
                input.DaprAddress,
                input.StateStore ?? throw new InvalidDataException("No state store name was specified."),
                input.Key ?? throw new InvalidDataException("No state store key was specified."),
                cancellationToken);
            return stateRecord;
        }
    }
