...
{
    internal abstract class DaprTriggerBindingBase : ITriggerBinding
    {
        private readonly DaprServiceListener serviceListener;
        private readonly ParameterInfo parameter;

        protected DaprTriggerBindingBase(DaprServiceListener serviceListener, ParameterInfo parameter)
        {
            ...
            this.BindingDataContract = new Dictionary<string, Type>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { parameter.Name ?? string.Empty, parameter.ParameterType },
            };
        }
        ...
        async Task<ITriggerData> ITriggerBinding.BindAsync(object value, ValueBindingContext context)
        {
            ...
            else if (destinationType == typeof(CloudEvent))
            {
                convertedValue = await ConvertCloudEventFromJson(inputStream, context);
            }
            else
            {
                var jsonValue = await CreateJToken(inputStream, context);
                ...
                else
                {
                    ...
                    convertedValue = ConvertFromJson(jsonValue, destinationType);
                }
            }

            // Support for basic expression binding
            bindingData[this.parameter.Name ?? string.Empty] = convertedValue;
        ...
        protected virtual async Task<object?> ConvertCloudEventFromJson(Stream inputStream, ValueBindingContext context)
        {
            var jsonValue = await CreateJToken(inputStream, context);
            return ConvertFromJson(jsonValue, typeof(CloudEvent));
        }
        ...
        private async Task<JToken> CreateJToken(Stream inputStream, ValueBindingContext context)
        {
            JToken jsonValue;
        ...
        private class HttpContextReturnValueBinder : IValueBinder
        {
